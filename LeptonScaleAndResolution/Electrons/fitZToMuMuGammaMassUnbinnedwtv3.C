 ...
 *
 * $Id: fitZToMuMuGammaMassUnbinnedwtv3.C,v 1.1 2012/10/24 15:41:26 yangyong Exp $
 *
 *
...
void fitZToMuMuGammaMassUnbinnedv3(
					float &dm_fit,
					float &dm_fitErr,
					float &a_fit,
				     float &n_fit,
...
// Di-photon mass signal p.d.f.
  ...
  //model = new RooAddPdf ("model", "signal + background mass model", RooArgList(BWxCB), RooArgList(nsig));
  //  if(IsData ==1){
  
  BWxCB.fitTo(*dataset,SumW2Error(kTRUE));

  //  BWxCB.fitTo(*dataset,Strategy(2),FitOptions("mh"),SumW2Error(kTRUE));
  

  //BWxCB.fitTo(*dataset,Strategy(2),FitOptions("mh"),SumW2Error(kTRUE),Optimize(0),Timer(1));
  ...
  if( usecdf){
   RooAbsReal *cdf = BWxCB.createCdf(*mass);
  ...
  //return;
  ///RooRealVar *clonedmass = (RooRealVar*)BWxCB.getDependents(&RooArgSet(*mass))->find("mass");
  double mmax = 0.0;
  double pmax = 0.0;
    double mleft = 0.0;
    double mright = 0.0;
    ...
    for (int i=0; i<Nstep; ++i) {
      ...
      mass->setVal(mlow);
      double pval = BWxCB.getVal(&RooArgSet(*mass));
      if (pval>pmax) {
        pmax = pval;
        mmax = mlow;
      }
