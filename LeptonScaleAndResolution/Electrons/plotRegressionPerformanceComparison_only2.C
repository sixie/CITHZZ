//================================================================================================
//			plotRegressionPerformance_only2.C
// 
// Plots, for comparison, the performance of two different regressions for electron energy
// To be used with trainElectronEnergyRegression_option.C and applyElectronEnergyRegression_option.C
//
// USAGE
// plotRegressionPerformanceComparison_only2(string applyingFilename, string targetFilename1, string targetFilename2, 
//				       string versionOption1, string versionOption2,
...
//*************************************************************************************************
void plotRegressionPerformanceComparison_only2(string applyingFilename, string targetFilename1, string targetFilename2, string versionOption1, string versionOption2, string outDirectoryName, Int_t Option) {  
  ...
  for (int j = 0; j < nFiles; j++){
    ...
    string versionName;
    if (versionOptions[j] == "V00") versionName = "no pt split, no trak vars"; 
    if (versionOptions[j] == "V01") versionName = "no pt split, trak vars"; 
    if (versionOptions[j] == "V10") versionName = "pt split, no trak vars"; 
    if (versionOptions[j] == "V11") versionName = "pt split, trak vars"; 
    if (versionOptions[j] == "V5")  versionName = "no pt split, trak vars and trak error";
  ...
  cv->SaveAs((outDirectoryName + "EnergyResponse_binOption" + OptionStr + ".gif").c_str());
  cv->SaveAs((outDirectoryName + "EnergyResponse_binOption" + OptionStr + ".pdf").c_str());
  cv->SaveAs((outDirectoryName + "EnergyResponse_binOption" + OptionStr + ".root").c_str());
} 
