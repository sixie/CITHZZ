...
// $Id: RecoAnalyzer.cc,v 1.1 2013/02/21 13:59:20 sixie Exp $
//
//
 
#include "CITHZZ/RecoAnalyzer/interface/RecoAnalyzer.h"
...

{
  ...
  m_genJetsSrc = iConfig.getUntrackedParameter<edm::InputTag>("genJets",edm::InputTag("ak5GenJets"));
  ...
  printWarning = iConfig.getUntrackedParameter<bool>("printWarning",false);
  applyGenLevelZPlusJetsM4LFilter = iConfig.getUntrackedParameter<bool>("applyGenLevelZPlusJetsM4LFilter",false);
...
RecoAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  Analysis->Branch("nGenJets",&nGenJets,"nGenJets/I");
  Analysis->Branch("etaGenJet",etaGenJet,"etaGenJet[nGenJets]/F");
  Analysis->Branch("phiGenJet",phiGenJet,"phiGenJet[nGenJets]/F");
  Analysis->Branch("ptGenJet",ptGenJet,"ptGenJet[nGenJets]/F");
  Analysis->Branch("massGenJet",massGenJet,"massGenJet[nGenJets]/F");
  Analysis->Branch("hadEnergyGenJet",hadEnergyGenJet,"hadEnergyGenJet[nGenJets]/F");
  Analysis->Branch("emEnergyGenJet",emEnergyGenJet,"emEnergyGenJet[nGenJets]/F");
  Analysis->Branch("invisibleEnergyGenJet",invisibleEnergyGenJet,"invisibleEnergyGenJet[nGenJets]/F");
  Analysis->Branch("auxiliaryEnergyGenJet",auxiliaryEnergyGenJet,"auxiliaryEnergyGenJet[nGenJets]/F");
  ...
  if( !isRealData){
	...
	nMCpart++;
      ...
      nWarning ++; 
      if(printWarning && nWarning<100) cout<<"genParticles not working.."<<endl;
    }   
  ...
  Handle<GenJetCollection> genJet;
    try {
      iEvent.getByLabel(m_genJetsSrc,genJet);
      
      nGenJets = 0; 
      for(int i=0; i< int(genJet->size())&& nGenJets<MAXGenSaved; i++){
	const GenJet & p = (*genJet)[i];

        //******************************************
        //Skip genjets that match to gen leptons
        bool matchGenLepton = false;
        for( int l=0; l< nGenEle; ++l) {
          if (!(statusGenEle[l] == 3)) continue;
          if (GetDeltaR(etaGenEle[l],p.eta(),phiGenEle[l],p.phi()) < 0.1) matchGenLepton = true;
        }
        for( int l=0; l< nGenMu; ++l) {
          if (!(statusGenMu[l] == 3)) continue;
          if (GetDeltaR(etaGenMu[l],p.eta(),phiGenMu[l],p.phi()) < 0.1) matchGenLepton = true;
        }
        if (matchGenLepton) continue;
        // ******************************************


	eGenJet[nGenJets] = p.energy();
	pGenJet[nGenJets] = p.p();
	pxGenJet[nGenJets] =p.px();
	pyGenJet[nGenJets] =p.py();
	pzGenJet[nGenJets] =p.pz();
	ptGenJet[nGenJets] =p.pt();
	etaGenJet[nGenJets] = p.eta();
	phiGenJet[nGenJets] = p.phi();
	massGenJet[nGenJets] = p.mass();

	hadEnergyGenJet[nGenJets] = p.hadEnergy();
	emEnergyGenJet[nGenJets] = p.emEnergy();
	invisibleEnergyGenJet[nGenJets] = p.invisibleEnergy();
	auxiliaryEnergyGenJet[nGenJets] = p.auxiliaryEnergy();
	
	nGenJets++;
      }
    }catch(std::exception& ex ){
      nWarning ++; 
      if(printWarning && nWarning<100) cout<<"genJets not working.."<<endl;
    }   
    
  /////////// =================END OF Gen Jet information ================ ////////////////////

  /////////// =================END OF Gen Jet information ================ ////////////////////

    if (applyGenLevelZPlusJetsM4LFilter) {

      int nGenEleFromZ = 0;
      int nGenMuFromZ = 0;
      
      TLorentzVector *ZLepton1 = 0;
      TLorentzVector *ZLepton2 = 0;
  ...
  if( !isRealData){
	...
	pileupTrueNumInterations = PVI->getTrueNumInteractions(); ///Only >= 4_2_8 Fall11MC
      ...
      nWarning ++; 
      if(printWarning && nWarning<100) cout<<"PileupSummaryInfo not working.."<<endl;
    }
  }
  ...
  try{
    ...
    nWarning ++; 
    if(printWarning && nWarning<100) cout<<"electron  not working.."<<endl;
  }
  ...
  try{
    ...
    nWarning ++; 
    if(printWarning && nWarning<100) cout<<"muon  not working.."<<endl;
  }
  ...
  try{
    ...
    nWarning ++; 
    if(printWarning && nWarning<100)cout<<"photon  not working.."<<endl;
  }
